...
document.addEventListener('DOMContentLoaded', () => {
  ...
  class Particle {
    constructor(w, h) {
      ...
      this.color = `rgba(0,255,150,${this.alpha})`;
      
      // Random color variations for non-image particles
      if (!this.useImage) {
        const hue = Math.random() > 0.5 ? 140 : 280; // Green or purple
  ...
  class CursorParticle {
    constructor(x, y) {
      ...
      this.life = 1.0;
      this.color = `hsla(${Math.random() * 60 + 120}, 100%, 70%, ${this.life})`;
    }
    update(dt) {
    ...
    for (let i = 0; i < bgParticles.length; i++) {
      ...
      if (!reducedMotion && mouse.x !== null && mouse.y !== null) {
        ...
        if (dist < mouse.radius) {
          ...
          bgCtx.strokeStyle = `rgba(0,255,150,${alpha})`;
          bgCtx.lineWidth = 0.3;
          bgCtx.stroke();
        }
      }
  ...
  if (window.gsap && !reducedMotion) {
    try {
      ...
      if (heroTitle) {
        ...
        gsap.to(heroTitle, {
          textShadow: '0 0 20px var(--secondary-neon-purple), 0 0 30px var(--secondary-neon-purple)',
          repeat: -1,
          yoyo: true,
          duration: 2,
          ease: 'sine.inOut'
  ...
  class Obstacle {
    constructor(canvas) {
      ...
      this.color = `hsl(${Math.random() * 60 + 270}, 80%, 50%)`;
  ...
  function gameLoop(ts) {
    ...
    for (let i = 0; i < stars.length; i++) {
      ...
      if (player && collides(player, star)) {
        ...
        createExplosion(star.x + star.width/2, star.y + star.height/2, 10, '#ffff00');
  ...
  function onGameTouchStart(e) {
    ...
    createExplosion(player.x + player.width / 2, player.y + player.height, 3, '#00ff00');
  }
  ...
  function onGameTouchMove(e) {
    ...
    if (performance.now() % 5 === 0) {
      createExplosion(player.x + player.width / 2, player.y + player.height, 1, '#00ff00');
    }
  }
  ...
  if (isTouchDevice && gameCanvas) {
    ...
    touchHint.style.color = 'rgba(0, 255, 0, 0.7)';
